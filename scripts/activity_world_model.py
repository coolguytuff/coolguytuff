...
def parse_cells(svg_text: str) -> list[Cell]:
    ...
    class_to_level = {
        ...
        for m in re.finditer(r'\.c\.([A-Za-z0-9_-]+)\s*\{[^{}]*?fill\s*:\s*var\(--c([0-4])\)', svg_text, re.S)
    }
    try:
    ...
    for el in root.iter():
        ...
        if inline:
            level = int(inline.group(1))
        try:
            x = float(el.attrib.get('x', 'nan'))
            y = float(el.attrib.get('y', 'nan'))
        except ValueError:
            continue
        if not (math.isfinite(x) and math.isfinite(y)) or abs(x) > 100000 or abs(y) > 100000:
            continue
...
def group_weeks(cells: list[Cell]) -> list[Week]:
    ...
    for c in cells:
        x = round(c.x, 3)
        y = round(c.y, 3)
        grouped[x][y] = max(grouped[x].get(y, 0), c.level)
...
def build_route(weeks: list[Week]) -> list[RouteStep]:
    ...
    route = [RouteStep(i, i + 1, state) for i, state in enumerate(classify_steps(heights))]
    for ri, state in enumerate(classify_steps(list(reversed(heights)))):
        source = len(weeks) - 1 - ri
...
def position(index: int, week: Week, count: int) -> tuple[float, float]:
    ...
    raise_y = 8.0 * math.sin((index / max(1, count - 1)) * math.pi)
...
def camera_values(frames: list[Frame], count: int) -> str:
    ...
    vals = []
    for f in frames:
        cx = max(min_x, min(0.0, CANVAS_W * .46 - f.x))
        foot_plane = f.y + RUNNER_FOOTLINE
        airborne = max(0.0, BASE_Y - foot_plane - 175.0)
        cy = min(24.0, airborne * .10)
...
def _append(frames, dt, x, y, state, direction, sx=1.0, sy=1.0, phase=0.0, contact='none'):
    ...
    frames.append(Frame(t, x, y, state, direction, sx, sy, phase, contact))
    ...
    if state == 'climb':
        ...
        for i in range(1, samples + 1):
            ...
            if p < .22:
                ...
                q = _ease((p - .50) / .32)
                x = ledge_x + (x1 - ledge_x) * q
                y = y0 + (y1 - y0) * q - 5.0 * math.sin(q * math.pi)
                contact = 'hands'
            else:
                q = _ease((p - .82) / .18)
                x = x1
                y = y1 - 3.0 * math.sin(q * math.pi)
                contact = 'both'
...
def build_frames(weeks: list[Week]):
    ...
    surface_pos = [position(i, w, len(weeks)) for i, w in enumerate(weeks)]
    pos = [(x, y - RUNNER_FOOTLINE) for x, y in surface_pos]
    frames = [Frame(0, *pos[0], 'idle', 1, phase=0, contact='both')]
    arrivals = defaultdict(list)
    _append(frames, .50, *pos[0], 'idle', 1, phase=0, contact='both')
    cycle = 0
